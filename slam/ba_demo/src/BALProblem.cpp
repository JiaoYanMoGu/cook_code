...
void BALProblem::add_noise(double sigma_point, double sigma_cam)
{
    // for simplify, just add noise to camera position
    // and point position

    for(int i = 0 ; i < data_.NumPoints(); i++)
    {
        PerturbPoint3(sigma_point,data_.GetPoint(i));
    }

    for(int i = 0 ; i < data_.NumCameras();i++)
    {
        PerturbPoint3(sigma_cam,&(data_.GetCamera(i)[3]));
    }
}
