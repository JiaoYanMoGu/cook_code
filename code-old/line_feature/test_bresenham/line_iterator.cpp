...
MLineIterator::MLineIterator(const double x1_, const double y1_, const double x2_, const double y2_,const int next_step )
    ...
    error = dx / 2.0;
    ystep = (y1 < y2) ? next_step : -next_step;
...
bool MLineIterator::getNext(std::pair<int, int> &pixel) {
